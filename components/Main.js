...
export default class Main extends Component {
  constructor(props){
    super(props);
    this.botIsplaying = this.botIsplaying.bind(this);
  ...
  botIsplaying(index){
    ...
    data.push(index);
    this.setState({player: data, allZells: [
      {index: 0, val:''},
      {index: 1, val:''},
      {index: 2, val:'O'},
      {index: 3, val:''},
    ...
    if(this.state.player.length >= 3) {
      ...
      allComb.some( (item,i) => {
        if (this.getAllWinComb(winningCombinations).indexOf(item.join('')) !== -1) {
          alert('Player wins');
        }
      })
      // otherwise lets bot play
    ...
    return (
        <View style={styles.container}>
          {this.state.allZells.map((zell,i) => 
            <Zell key={zell.index} index={zell.index} value={zell.val} botplays={this.botIsplaying}/>
          )}
        </View>
    );
  }
